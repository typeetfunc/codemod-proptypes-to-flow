...
function propTypeToFlowType(j, key, value) {
  ...
  var TRANSFORM_MAP = {
    ...
    node: j.unionTypeAnnotation([j.numberTypeAnnotation(), j.stringTypeAnnotation(), reactElement, j.genericTypeAnnotation(j.identifier('Array'), j.typeParameterInstantiation([j.unionTypeAnnotation([j.numberTypeAnnotation(), j.stringTypeAnnotation(), reactElement])]))])
  };
  ...
  if (node.object) {
    node = j.memberExpression(getPropTypeExpression(node.object), node.property);
  } else if (node.callee) {
    node = j.callExpression(getPropTypeExpression(node.callee), node.arguments);
  }

  if (node.type === 'Literal') {
    returnValue = j.stringLiteralTypeAnnotation(node.value, node.raw);
  } else if (node.type === 'MemberExpression') {
    returnValue = declarators[node.property.name] ? j.identifier(declarators[node.property.name]) : TRANSFORM_MAP[node.property.name];
    ...
    if (name === 'instanceOf') {
      ...
      returnValue = j.genericTypeAnnotation(j.identifier('Array'), j.typeParameterInstantiation([propTypeToFlowType(j, null, node.arguments[0] || j.anyTypeAnnotation(), declarators)]));
    } else if (name === 'objectOf') {
      // TODO: Is there a direct Flow translation for this?
      returnValue = j.genericTypeAnnotation(j.identifier('Object'), j.typeParameterInstantiation([propTypeToFlowType(j, null, node.arguments[0] || j.anyTypeAnnotation(), declarators)]));
    } else if (name === 'shape') {
    ...
    returnValue = declarators[node.name] ? j.identifier(declarators[node.name]) : null;
  }

  returnValue = returnValue || (0, _fixMeType2.default)(j);
